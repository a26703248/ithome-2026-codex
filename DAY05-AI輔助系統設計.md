# Day 05｜AI 輔助系統設計：讓 ChatGPT 攤開選項，不替團隊拍板

![Day 05 封面：先比較代價，再決定架構](./%E5%9C%96%E6%AA%94/Day05/day05-01-cover.png)

Day 04 第二輪加入兩個會影響架構的示範條件：每日約 `1 TB`，資料來源速率約 `12 MB/s`。若兩者都以十進位計算，而且 `12 MB/s` 是可持續的端到端有效速率，光是傳輸就超過 23 小時。這時若把整批工作綁在同步的超文字傳輸協定（Hypertext Transfer Protocol，HTTP）請求裡，逾時、重送與部分失敗都會變得難處理；但因此立刻拆成多個服務，也可能只是增加維運負擔。

## 先給限制，再請 ChatGPT 提方案

我把 Day 04 的需求書、示範條件與未知資訊放在同一個 ChatGPT 專案裡。ChatGPT 專案能集中對話、檔案與專案指示，不過「放進專案」不代表內容已經正確，我仍要分開文件事實、示範輸入與工程推論。

![先把設計限制與未知資訊放進同一張圖](./%E5%9C%96%E6%AA%94/Day05/day05-02-constraints.png)

我沒有問「請給我最佳架構」，而是要求它用相同維度比較三個候選方案。完整提示詞放在 [Day 05 架構比較提示詞](https://github.com/a26703248/ithome-2026-codex/blob/main/%E7%A8%8B%E5%BC%8F%E7%A2%BC/DAY05/architecture-prompt.txt)，核心限制如下：
...
![三個候選架構的責任邊界](./%E5%9C%96%E6%AA%94/Day05/day05-03-options.png)
...
![第一版資料流：API 建立任務，背景工作元件執行匯入](./%E5%9C%96%E6%AA%94/Day05/day05-04-selected-flow.png)
...
![架構決策紀錄：現在採用、暫緩項目與驗證門檻](./%E5%9C%96%E6%AA%94/Day05/day05-05-decision-record.png)
...
這次留下的不是 ChatGPT 對方案的排名，而是可核對的比較欄位與重評門檻。Day 06 會把這次提示詞拆開，說明我如何交代目標、背景、輸入、限制、輸出格式與驗收方式。
