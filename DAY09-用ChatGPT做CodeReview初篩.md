...
我只問每一點能否轉成失敗斷言；無法重現的先留問題單，不要求作者改碼。
...
這份模擬初篩先攤開可能的失敗條件，JUnit 5 與官方文件留下可重現證據，人工審查再補上遺漏脈絡。從 Day 04 到今天，同一份需求已由問題清單走到能執行的驗證；Day 10 改由 Codex 進入工作目錄，觀察代理如何完成一輪實際任務。
