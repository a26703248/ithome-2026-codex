...
Day 21 已確認日報服務會逐位讀資料、產生可攜式文件格式（Portable Document Format，PDF），再呼叫郵件閘道。需求書另有一條未查明的舊問題：客戶增加後，曾有報表接近發送時間才開始，導致部分信件延遲。我第一個直覺是資料量拖慢計算；但直覺只能當假設，不能直接寫進根因欄。為了示範方法，我先建立一個縮小假設：同步寄信會不會擋住後面客戶的報表？
