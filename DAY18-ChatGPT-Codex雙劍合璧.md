...
Day 17 我先誠實列出這套日報服務的現況與新需求之間的落差。把 Day 16 的交付方式套進來準備動手時，第一個碰到的還是規格落差：業務列了四種頻率，以及可攜式文件格式（Portable Document Format，PDF）、Microsoft Word、Microsoft Excel 三種檔案格式，程式端卻有六項問題沒定案。Codex 若只看到功能清單，可能直接改動耦合的舊流程。此時缺的不是更多提示詞，而是哪些決策成立、何處必須停手。
