...
![Day 02 封面：同一項任務，依工作階段選擇工具](./%E5%9C%96%E6%AA%94/Day02/day02-01-cover.png)

Day 01 用「功能好像怪怪的」帶出問題，今天我把這個案例繼續拆開。我若立刻改程式，可能先猜錯問題；若只請 ChatGPT 提供修正碼，還要整合與驗證。兩種做法對應工作階段：前者需要釐清未知，後者需要在專案中完成已知任務。

## 先建立兩種工作心智模型

ChatGPT 是以對話為核心的通用助手，適合追問、整理、比較與產出內容；依方案與設定，也能搜尋網路與分析檔案。Codex 則是編程代理，可以在授權的本機或雲端環境中瀏覽程式庫、修改檔案、執行命令與測試，再回報差異與結果。這類環境可用沙箱限制檔案、命令與網路權限，讓任務隔離執行。

![ChatGPT 與 Codex 的工作心智模型比較](./%E5%9C%96%E6%AA%94/Day02/day02-02-mental-model.png)

| 比較維度 | ChatGPT | Codex |
|---|---|---|
| 主要互動 | 多輪對話、整理想法 | 以任務驅動專案操作 |
| 上下文 | 對話、上傳檔案、可用工具 | 指定專案、檔案與執行環境 |
| 主要產出 | 分析、草稿、規格與建議 | 程式碼差異、命令與測試結果 |
| 驗證方式 | 核對來源、需求與推論 | 檢查差異、測試與執行紀錄 |
| 常見風險 | 補出不存在的前提 | 依錯誤邊界完成看似正確、實際偏題的修改 |

隨者時間的演進功能會持續改變，所以以上是我目前整理個人工作流程和習慣上比較出來的，若有個人觀點皆以自己的判斷為主。

## 任務一：模糊需求先交給 ChatGPT 釐清
...
![從模糊回報整理成可驗證的 Issue Report](./%E5%9C%96%E6%AA%94/Day02/day02-03-requirement-clarification.png)
...
![Codex 從重現失敗到測試通過的驗證流程](./%E5%9C%96%E6%AA%94/Day02/day02-04-codex-verification.png)

重跑後三項測試全部通過，既有分類篩選測試也通過。不過，前後空白、特殊字元與大量資料效能尚未驗證，不能因畫面顯示成功就推論所有需求都已完成。

## 選擇工具：當前資訊是否可以被描述出狀況來

![選擇工具：看下一步需要什麼證據](./%E5%9C%96%E6%AA%94/Day02/day02-05-decision-tree.png)
